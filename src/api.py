...
from src.data_loader import create_feature_bundle, create_sliding_windows
...
class CvService:
    ...
    def run(self, source: str, path: Optional[str] = None, max_seconds: Optional[int] = None, frame_stride: Optional[int] = None) -> Dict[str, float]:
        ...
        if source == "camera":
            ...
            video_path = path or os.getenv("CV_VIDEO_PATH", "D:/OnTime/temp_video.mp4")
        ...
        out_path = os.path.join("frontend-nextjs", "public", "cctv-detected.mp4")
    ...
    def _worker():
        try:
            if source == "camera":
                ...
                vp = path or os.getenv("CV_VIDEO_PATH", "D:/OnTime/temp_video.mp4")
